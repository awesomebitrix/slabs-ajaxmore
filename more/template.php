...
<?=$arResult["NavFirstRecordShow"]?> <?=GetMessage("nav_to")?> <?=$arResult["NavLastRecordShow"]?> <?=GetMessage("nav_of")?> <?=$arResult["NavRecordCount"]?><br /></font>
    ...
    <div>
        <?if( $arResult["NavPageNomer"] != $arResult["NavPageCount"]){?>
            <span
                class="sl-more"
                data-url="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]+1)?>"
                data-current="<?=$arResult["NavPageNomer"]?>"
